...
PLUGIN_API int iStickDistance = 0;//exporting to share the distance we're sticking.
    ...
    while (*szCurrentArg)
    {
        ...
        else if (ucCmdUsed == CMD_STICK)
        {
            ...
            else if (isdigit(szCurrentArg[0]) || szCurrentArg[0] == '.' )
            {
                ...
                {
                    STICK->Dist = iStickDistance = (float)atof(szCurrentArg) * STICK->DistModP + STICK->DistMod;
                }
                STICK->SetDist = true;
                STICK->TurnOn();
            }
...
void MainProcess(unsigned char ucCmdUsed)
{
    ...
    if (ucCmdUsed == CMD_STICK)
    {
        if (!STICK->SetDist)
        {
            STICK->Dist = iStickDistance = (psTarget->StandState ? get_melee_range(pLocalPlayer, (PlayerClient*)psTarget) : 15.0f) * STICK->DistModP + STICK->DistMod;
            STICK->SetDist = true;
        }
